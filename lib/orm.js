...
const DeleteFromServiceTimeMaker = require("./delete_from_service_time_maker.js");
...
class ORM extends SuperORM {
    ...
    getDeleteFromOtherDocument(action) {
        let orm = new DeleteFromOtherDocumentORM(this.req, this.res, action);
    ...
    getOtherDocuments() {
        let orm = new OtherDocumentsORM(this.req, this.res);
...
class IndexORM extends SuperORM {
    ...
    fetchDeacon() {
        const queryString =
            ...
            "FROM ParishRole " +
            "JOIN Contact ON Contact.code = ParishRole.contact " +
            "LEFT JOIN RealWorldAddress ON RealWorldAddress.code = " +
            "Contact.address " +
            "WHERE ParishRole.code = 'deacon';";

        this.runQuery(queryString, [], "fetchDeaconPart2", []);
    }

    fetchDeaconPart2(extract) {
        this.data.deacon = objectifyExtract(extract);

        this.fetchParishChurches();
    }
    ...
    fetchParishChurches() {
        const queryString =
            ...
            "WHERE is_parish_church IS TRUE " +
            "ORDER BY short_name;";

        this.runQuery(queryString, [], "fetchParishChurchesPart2", []);
    }

    fetchParishChurchesPart2(extract) {
        this.data.parishChurches = objectifyExtract(extract);

        this.fetchLinkedChurches();
    }
    ...
    fetchLinkedChurches() {
        const queryString =
            "SELECT * " +
            "FROM RealWorldAddress " +
            "WHERE is_linked_church IS TRUE " +
            "ORDER BY short_name;";

        this.runQuery(queryString, [], "fetchLinkedChurchesPart2", []);
    }

    fetchLinkedChurchesPart2(extract) {
        this.data.linkedChurches = objectifyExtract(extract);
...
class Upload2ServiceTimeORM extends SuperORM {
    ...
    fetchRealWorldAddresses() {
        const queryString =
            "SELECT code, short_name FROM RealWorldAddress;";
...
class DeleteFromNewsletterORM extends SuperORM {
    ...
    wrapUp() {
        const properties = {
            ...
            newsletterSummaries: makeNewsletterSummaries(this.data.newsletters),
        };
...
// Gather the necessary data for the other documents page.
class OtherDocumentsORM extends SuperORM {
    constructor(req, res) {
        super(req, res);
    }
    ...
    fetchOtherDocuments() {
        const queryString =
            "SELECT * FROM OtherDocument ORDER BY publication_date;";

        this.runQuery(queryString, [], "fetchOtherDocumentsPart2", []);
    }

    fetchOtherDocumentsPart2(extract) {
        this.data.otherDocuments = extract;
    ...
    wrapUp() {
        const properties = {
            ...
            otherDocuments: this.data.otherDocuments,
        };

        this.finaliser.protoRender(
            this.req, this.res, "other_documents", properties
        );
    }
}

// Gather the necessary data for the delete other documents form.
class DeleteFromOtherDocumentORM extends SuperORM {
    constructor(req, res, action) {
        super(req, res);
        this.action = action;
    }
    ...
    fetchOtherDocumentsPart2(extract) {
        this.data.otherDocuments = objectifyExtract(extract);
    ...
    wrapUp() {
        const properties = {
            ...
            formAction: this.action,
            otherDocumentSummaries: makeOtherDocumentSummaries(
                this.data.otherDocuments
            ),
        };
...
class WebmasterORM extends SuperORM {
    ...
    wrapUp() {
        ...
        this.finaliser.protoRender(this.req, this.res, "webmaster", properties);
    }
}
        ...
        for (let i = 0; i < this.data.pilgrimages.length; i++) {
            ...
            if (row.end_year) {
                ...
                if (
                    ...
                    (row.end_year === currentYear)
                ) {
                    if (row.end_month < currentMonth) continue;

                    if (
                        (row.end_month === currentMonth) &&
                        (row.end_day < currentDay)
                    ) {
                        continue;
                    }
                }
            ...
            pilgrimage.start = MUtils.makeDayMonthString(
                row.start_day,
                row.start_month
            );
            pilgrimage.end = MUtils.makeDayMonthString(
                row.end_day,
                row.end_month
            );
...
class YouthORM extends SuperORM {
    ...
    fetchChildrensLiturgiesPart2(extract) {
        this.data.childrensLiturgies = extract;
    ...
    wrapUp() {
        let properties = {
            title: "Youth",
            childrensLiturgies: this.childrensLiturgies,
        };

        this.finaliser.protoRender(this.req, this.res, "youth", properties);
    }
}
...
function makeOtherDocumentSummaries(data) {
    let result = [];
    let description, summary, objectifiedRow, weekBeginning;
    ...
    for (let i = 0; i < data.rows.length; i++) {
        objectifiedRow = MUtils.objectifyRow(data.columns, data.rows[i]);
        description = JSON.stringify(objectifiedRow);
        summary = { id: objectifiedRow.id, description: description };
        result.push(summary);
    }
